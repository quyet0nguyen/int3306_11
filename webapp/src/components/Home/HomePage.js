...
class HomePage extends Component {
    ...
    callApi = async () => {
        let response = await fetch('/home').then((response)=>response.json())
                        .then(data =>{
    ...
    render() {
        ...
        const days =['Sunday','Monday','Tueday','Wednesday','Thusday','Saturday']
       	let d = new Date();
        return (
        	<div>
        	<h2 style={{color:'#757575'}}> Today is {days[d.getDay()]}, {d.getDate()}  {d.getMonth()}  {d.getFullYear()}</h2>
            <Fragment>
                <MaterialTable
                    icons={tableIcons}
                    title="ACTIVE"
                    columns={columns}
                    data={this.state.class[0]}/>
            </Fragment>
            <br/> <br/>
            <Fragment>
                <MaterialTable
